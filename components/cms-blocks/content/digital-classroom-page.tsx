...
const defaultFeatures: FeatureItem[] = [
  { text: 'Digital classrooms with latest technology', icon: 'Monitor' },
...
function getIconComponent(iconName: string) {
  const icons: Record<string, typeof Monitor> = {
    Monitor, Wifi, Presentation, Video, Users,
    CheckCircle2, GraduationCap, Lightbulb, Laptop,
  }
...
export default function DigitalClassroomPage({
  ...
  // Combine heroImage + images array into a single list
  const allImages: { src: string; alt?: string }[] = []
  if (images.length > 0) {
    allImages.push(...images.filter((img) => img.src))
  } else if (showHeroImage && heroImage) {
    allImages.push({ src: heroImage, alt: heroImageAlt })
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {allImages.map((image, idx) => (
                ...
                >
                  <Image
                    src={image.src}
                    alt={image.alt || `Classroom image ${idx + 1}`}
                    fill
