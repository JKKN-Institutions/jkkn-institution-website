      ...
      >
        ...
        {hasChildren && isOpen && (
          <div
            id={`dropdown-${item.id}`}
            className={cn(
              "absolute top-full w-56 origin-top z-50",
          ...
          >
            ...
            <div className="h-2" />
            <div className="bg-white rounded-lg shadow-xl shadow-black/15 border border-gray-200 py-1 max-h-[calc(100vh-10rem)] overflow-x-hidden overflow-y-auto animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-300 ease-out">
      ...
      {hasChildren ? (
        <>
          <button
            ...
            className={cn(
              'w-full flex items-center justify-between px-3 py-1.5 text-sm transition-all duration-200',
              isOpen
          ...
          {/* Flyout dropdown for nested level */}
          {isOpen && (
            <div
              id={`dropdown-${item.id}`}
              className="absolute left-full top-0 w-56 origin-top-left z-50"
              role="menu"
              aria-label={item.label}
              onMouseEnter={() => onHover?.(item.id, parentPath)}
              onMouseLeave={(e) => {
                ...
                const currentTarget = e.currentTarget as HTMLElement
                if (!relatedTarget || !(relatedTarget instanceof Node) || !currentTarget.contains(relatedTarget)) {
                  onHover?.(null, currentPath)
                }
              }}
            >
              {/* Invisible bridge to prevent gap hover issues */}
              <div className="absolute right-full top-0 w-2 h-full" />
              <div className="bg-white rounded-lg shadow-xl shadow-black/15 border border-gray-200 py-1 max-h-[calc(100vh-10rem)] overflow-x-hidden overflow-y-auto animate-in fade-in zoom-in-95 slide-in-from-left-2 duration-300 ease-out">
                {item.children!.map((child) => (
                  <NavDropdownItem
                    key={child.id}
                    item={child}
